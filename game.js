document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================================================
    // BANCO DE DADOS DO JOGO (COM SUAS CUSTOMIZAÇÕES E CORREÇÕES)
    // =========================================================================
    const produtosDB = {
    ...
    const missoesDB = {
        m001: { titulo: "Extermínio de Wasps", descricao: "Wasps estão causando problemas. Acabe com alguns.", objetivo: { tipo: 'DERROTAR', idAlvo: 'wasp', quantidade: 3 }, recompensa: { dinheiro: 100, xp: 50, marca: 5 } },
        m002: { titulo: "Mestre de Poções", descricao: "Para manter os aventureiros, produza algumas poções.", objetivo: { tipo: 'PRODUZIR', idAlvo: 'pocaoCura', quantidade: 5 }, recompensa: { dinheiro: 50, xp: 30 } },
        m003: { titulo: "Eliminar Devours", descricao: "Devours estão atacando novamente! Acabe com eles!!", objetivo: { tipo: 'DERROTAR', idAlvo: 'devour', quantidade: 3 }, recompensa: { dinheiro: 250, xp: 120, marca: 10 } }
    };
    
    // =========================================================================
    // ESTADO DO JOGO (COM SUA NOVA MISSÃO)
    // =========================================================================
    let estadoDoJogo = {
        heroi: { 
            nivel: 1, xp: 0, xpParaProximoNivel: 50,
            hp: 100, hpMax: 100, 
            energia: 50, energiaMax: 50,
            statsBase: { ataque: 10, defesa: 0 },
            equipamento: { arma: null, armadura: null }
        },
        inimigoAtual: null,
        dinheiro: 100,
        marca: 0,
        vendedores: { pocao: 0, custoProximoVendedor: 50 },
        inventario: { pocaoCura: 5, espadaMadeira: 1 },
        missoes: { m001: { progresso: 0, status: 'disponivel' }, m002: { progresso: 0, status: 'disponivel' }, m003: { progresso: 0, status: 'disponivel' }}
    };

    // =========================================================================
    // CAPTURA DOS ELEMENTOS DO DOM (UI)
    // =========================================================================
    const ui = {
        ...
        botaoPrincipal: document.getElementById('botao-acao-principal'),
        combateAcoes: document.getElementById('combate-acoes'),
        logEventos: document.getElementById('log-eventos'),
    ...
    // =========================================================================
    // LÓGICA DE JOGO (COMPLETA E FUNCIONAL)
    // =========================================================================
    ...
    function verificarProgressoMissao(tipo, id) { for (const idMissao in estadoDoJogo.missoes) { const mE = estadoDoJogo.missoes[idMissao]; const mD = missoesDB[idMissao]; if (mE.status === 'ativa' && mD.objetivo.tipo === tipo && mD.objetivo.idAlvo === id) mE.progresso++; } }
    ...
    function inicializar() {
        ...
        const modalFundo = document.getElementById('fundo-modal-loja');
        ...
        botaoAbrirLoja.addEventListener('click', () => modalFundo.classList.remove('hidden'));
        const fecharModal = () => modalFundo.classList.add('hidden');
        botaoFecharLoja.addEventListener('click', fecharModal);
